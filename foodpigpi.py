...
def main():
    ...
    def initialise_columns(name):
        worksheet = sh.worksheet(name)
        columns_list = worksheet.row_values(1)
        n = 0
        for i in range(len(columns_list)):
            # rename duplicate columns fot identification
    ...
    def join_response_and_canned_message(customer, drop):
        ...
        df = pd.melt(df)
        df = df[df.value != '']
        df = pd.merge(df, canned_message, how='inner', left_on='variable', right_on='Excel Name')
        return df
        ...
        for shop in pd.unique(customer_level.Shop):
            ...
            for category in pd.unique(shop_level.Category):
                ...
                for item in category_level['Whatsapp Name']:
                    message_middle += str(item) + ' x ' + str(category_level[category_level['Whatsapp Name'] == item].value.values[0]) + '\n'
    ...
    def calculate_price(customer):
        ...
        for bulk_dict in bulk_list_dict:
            ...
            bulk_discount += int(bulk_quantity/bulk_dict['quantity']) * bulk_dict['discount']
    ...
    def get_delivery_message_and_update_delivery_dict(customer):
        ...
        def merge_delivery(merge_value):
            if merge_value == "需要" and len(pickups) > 0:
                ...
                for pickup in unmerged_pickups:
                    ...
                    customer_level.loc[customer_level.Delivery == pickup,('Delivery')] = last_pickup
        ...
        pickups = [delivery_options.index(option) + 1 for option in delivery_options if option.startswith('工作室自取')]
        merge_delivery(responses.loc[customer][merge_column].values[0])
        delivery_options_message = ""
        for option in delivery_options:
            if option != "":
                delivery_options_message += option + '\n'
                delivery_option_list[delivery_options.index(option)].update({customer: option})
        # remove null delivery options before iteration
        delivery_options = [option for option in delivery_options if option != ""]
        for delivery in deliveries:
            delivery_message = ""
            delivery_level = customer_level[customer_level.Delivery == delivery]
            for item in delivery_level['Whatsapp Name']:
                delivery_message += demoji.replace(item, '') + ' x ' + str(delivery_level[delivery_level['Whatsapp Name'] == item].value.values[0]) + ' + '
    ...
    def update_worksheet():
        ...
        for i in range(len(deliveries)):
            ...
            delivery_item = pd.DataFrame.from_dict(delivery_item_list[i], orient='index')
            delivery_option = pd.DataFrame.from_dict(delivery_option_list[i], orient='index')
            df = pd.merge(delivery_item, delivery_option, left_index=True, right_index=True)
            df = df.reset_index()
            df.columns = ['IG Name', 'Items', 'Delivery']
            df = df.sort_values('Delivery')
            sh.worksheet(str(i + 1)).update([df.columns.values.tolist()] + df.values.tolist())
    ...
    if check_duplicate_customers(responses):
        print ('ERROR: duplicate customers')
        return
    responses = responses.set_index('IG 名稱~ (必需正確全名✅)')
    merge_column = [column for column in responses.columns if column.startswith('選擇了工作室自取 是否需要合併一同取貨?')]
    delivery_column = [column for column in responses.columns if column.startswith('您選擇的交收時間地點〜〜')]
    non_item_columns = (['Timestamp', '您是否同意以上事項說明? ', '聯絡電話〜(會whatsapp回覆訂單的)'] +
                        [column for column in responses.columns if column.startswith('請問需要訂購')] + 
                        ['如訂不到某一家/某項產品 就全單不要請下單後wtsapp pigpi~ (如不通知而棄單者 會列入黑名單!!)', '留言/查詢區~'] +
                        merge_column + delivery_column)
    ...
    for customer in responses.index:
        ...
        message_full = message_template.format(message_middle, delivery_message, price, payment_deadline)
        whatsapp_link = 'https://api.whatsapp.com/send/?phone={}&text={}'.format(phone_number, message_full.replace('\n','%0a').replace('&','%26'))
